...
const GuessSelector = ({guesses, guessSelectionHandler}: GuessSelectorProps) => {
    ...
    const selectionHandler = (e: MouseEvent<HTMLElement>) => guessSelectionHandler(guesses[parseInt(e.currentTarget.getAttribute('data-tag')!!)])
    return <div style={buttonStyle}>{guesses.map((guess, index) => <button key={index} data-tag={index} style={buttonStyle} onClick={selectionHandler}>{guess}</button>)} </div>
}
